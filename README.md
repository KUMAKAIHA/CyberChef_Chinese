...
CyberChef 主要包含四个区域：
 ...
 4. 位于中间的 **配方** 区域，您可以将所需操作拖入并指定参数和选项。
...
## 特性
 ...
 - 自动烘焙
     - 每当您修改输入或配方时，CyberChef 会自动“烘焙”并生成输出。
     - 如果输入数据量巨大，也可选择关闭自动烘焙，手动操作以提升性能。
 - 自动检测编码
 ...
 - 断点调试
     - 您可以在配方中的任何操作上设置断点，在执行前暂停操作。
     - 同时支持逐步执行配方，查看每个阶段的数据状态。
 - 保存与加载配方
     - 如果您设计出优秀的配方，可点击“保存配方”将其存储于本地，下次访问时自动加载。
     - 您也可以复制包含配方和输入的 URL，方便与他人共享。
 - 搜索
 ...
 - CyberChef 完全在客户端运行
     - 请注意，您的配方配置和输入（文本或文件）均不会发送到 CyberChef 服务器——所有处理均在您的浏览器中完成。
